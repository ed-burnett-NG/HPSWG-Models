...
- `//flowchart LR fix` forces a more rigid layout to minimise node overlaps; useful for models with longer property labels
...
- **`E73` text nodes in document models** are shown with a `literal` format tag as a diagram simplification. In the knowledge graph, a `P190_has_symbolic_content` triple would be required to hold the actual literal value. This is a known and accepted simplification across the document model family.
- **`E39 Actor`** is used in document models rather than `E21 Person` to accommodate both persons and organisations as contributors and authors. `//links` directives use `[confirmed]` to suppress hierarchy warnings where `E39` links to `person` or `organisation` model targets.
- **`idea_bn`** is a valid format class for `E28 Conceptual Object` blank nodes. It is used in the `simple_document` model for the `E28: Part Position` node.
...
## Document model family (new -- April 2026)

Three new interrelated models were developed in April 2026 to support semantic documentation of heritage science records. They are designed to be used together and are intentionally source-agnostic.

### Model overview

| Model | Version | Key entity | Notes |
|---|---|---|---|
| `simple_document` | v1.0 | `E31: Simple Document` | Core document model; links to document_creation and document_update |
| `document_creation` | v1.0 | `E65: Document Creation` | Creation provenance; links back to simple_document |
| `document_update` | v1.0 | `E13: Document Update` | Update provenance; links back to simple_document |

### Key design decisions for the document model family

- **`simple_document`** is the core model. It is intentionally generic and reusable across document types. Source-specific typing is handled via `E55: Document Type` and `E55: Domain Type` controlled lists.
- **`E31` can be composed of ordered parts** via `P148_has_component`. Parts are themselves `simple_document` instances, allowing figures, sections, and composite reports to be modelled within the same structure.
- **Part ordering** is handled via an `E28: Part Position` blank node (`idea_bn`) linked to both parent and part via two `P67_refers_to` triples. The `E28` carries an `E54: Part Order` dimension node. This pattern allows the same part to appear in multiple parent documents at different positions without conflict. A numeric literal value for the order is not stored directly on the `E54`; a separate literal structure would be required. This is a known limitation under discussion.
- **`P148_has_component`** is used for sub-documents (confirmed as correct for `E89 Propositional Object` composition). `P165_incorporates` is used for images incorporated into a document.
- **`E73` text nodes** are shown with a `literal` format tag as a diagram simplification. In the knowledge graph a `P190_has_symbolic_content` triple holds the actual literal. This is a known and accepted simplification.
- **Creation provenance** is captured via `document_creation` (`E65`), linked from `simple_document` via `P94i_was_created_by (0 to 1)`. Making this optional allows documents to exist without recorded creation provenance.
- **Update provenance** is captured via `document_update` (`E13`), linked from `simple_document` via `P140i_was_attributed_by (0 to n)`. Multiple updates accumulate in the graph; the current text is always the most recent `P190` value on the `E31`.
- **Versioning** is handled by creating a new `E31` via a new `document_creation` event, not by updating the existing `E31`. The new document carries an updated version number via `P43_has_dimension` on an `E54: Version Number` node. The link to the previous version is recorded on the new `E31` via `P130_shows_features_of`. Version links are undirected -- both older and newer versions can be linked from the same document. `P130_shows_features_of` is used as a pragmatic approximation for version succession; a more precise property may be adopted in future.
- **`E39 Actor`** is used throughout the document model family (rather than `E21 Person`) to accommodate both persons and organisations. Author in `document_creation`; Editor in `document_update`. Both link to `person` and `organisation` model targets with `[confirmed]`.
- **`document_creation`** records the author (`E39`), creation timespan (`E52`), and optionally the initial text (`E73`). Two optional influence links are available: `P15_was_influenced_by` for general reference or source documents, and `P130_shows_features_of` on the created `E31` for explicit previous version links.
- **`document_update`** records the editor (`E39`), edit scale (`E55: Edit Scale` -- controlled list: `major, minor, minimal`), update timespan (`E52`), updated text (`E73`), and the property being updated (`E55: Updated Property Type` -- fixed value: `P190_has_symbolic_content`) via `P177_assigned_property_type`. The `P177` and `P2` serve distinct purposes: `P177` identifies the property being assigned; `P2` records the scale of the edit.
- **No `E55: Creation Type`** on `document_creation` -- the `E65 Creation` class is semantically specific enough without a type node.
- **No `E55: Update Type`** on `document_update` -- version-creating changes are handled by a new `document_creation` event, not flagged within an update event.
- **`E13 Attribute Assignment` for part ordering vs document updates** -- these use the same class for different purposes. Part ordering uses `E13` as a relationship annotation mechanism (the `P177` value does not correspond to a real property on `E31`). Document updates use `E13` correctly to record a provenance-tracked property assignment. This distinction is noted in the `E28: Part Position` tooltip. The part ordering `E13` was subsequently replaced by the `E28` pattern, removing this ambiguity from `simple_document`.

### `simple_document` -- controlled lists

| Node | Controlled list values |
|---|---|
| `E55: Document Type` | `working_note, report_section, figure, composite_report, catalogue_entry` |
| `E55: Domain Type` | `x-radiography, infrared_reflectography, entry_text, provenance, condition, treatment, support, paint_layers, surface_coatings, bibliography, organic_analysis, cross_section` |
| `E55: Ordinal Position Type` | Fixed value: `ordinal_position` |

### `document_update` -- controlled lists

| Node | Controlled list values |
|---|---|
| `E55: Edit Scale` | `major, minor, minimal` |
| `E55: Updated Property Type` | Fixed value: `P190_has_symbolic_content` |
...
| `simple_document` | v1.0 | `E31: Simple Document` | New April 2026; see document model family section |
| `document_creation` | v1.0 | `E65: Document Creation` | New April 2026; see document model family section |
| `document_update` | v1.0 | `E13: Document Update` | New April 2026; see document model family section |
...
Next priority: `sample`, then remaining formed models. The new document model family (`simple_document`, `document_creation`, `document_update`) will need `//field` directives added once the models are stable.
...
Current overview does not include all domain models. Once consistency is confirmed, update to reflect the full inter-model connection set including the new document model family. May be partially automatable from `//links` directives.
...
### Part ordering pattern -- wider consultation needed
The current `E28: Part Position` pattern for ordering document parts is pragmatic but the use of `P67_refers_to` as a linking property is an approximation. A more semantically precise ordering pattern should be confirmed through wider consultation with the CIDOC CRM community. The `E54: Part Order` dimension currently does not store the numeric value directly; a literal structure needs to be agreed and added once the pattern is confirmed.

### `simple_document` -- `//field` directives not yet added
The `//field` directives were removed from `simple_document` during model redesign to focus on structure first. These need to be added back once the model is stable, following the standard directive syntax.
...
| `simple_descriptive_document` | **Superseded** | The `simple_document`, `document_creation`, and `document_update` models developed in April 2026 cover this scope. The precursor folder can be retired. |
...
- *"Here is the current simple_document / document_creation / document_update model, I want to continue working on it: [paste]"*
 
You do not need to re-explain the TSV format, the `//links` syntax, the `//field` and `//field-via` syntax, the README system, the three-section table structure, the dual-layer sample pattern, the `E22/S13` placeholder convention, or the document model family structure.
