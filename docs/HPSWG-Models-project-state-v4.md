...
HPSWG-Models/
  ...
  forms/
    ...
    README.template.md       # Human-authored intro and purpose
...
The internal delimiter within a `//field` line is `|`. The alternatives list uses `;` as separator. Commas must not be used as alternative label separators -- the Dynamic Modeller normalises comma-separated values to tabs during processing.
...
- **OPTIONAL NOTE**: optional fourth field; appended in brackets to the label description in the table
...
Only the `project` model (v1.2) currently has `//field` directives. The aim is to add directives to all formed models. Node labels should be reviewed and improved where needed as part of this work, so that the stripped label reads well as a plain-language field name without requiring alternatives to compensate.
